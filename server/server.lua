...
local function saveIdentityToDatabase(identifier, identity)
    MySQL.update.await(
        ...
        {identity.firstName, identity.lastName, identity.dateOfBirth, identity.sex, identity.height, identity.story, identifier})
end
...
local function checkIdentity(xPlayer)
    MySQL.single('SELECT firstname, lastname, dateofbirth, sex, height, story FROM users WHERE identifier = ?',
